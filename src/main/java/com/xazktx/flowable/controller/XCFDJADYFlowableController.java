...
@CrossOrigin
@RestController
@RequestMapping("XCFDJADY")
public class XCFDJADYFlowableController extends BaseController {
    ...
    public Result<String> qiDong() {
        ...
        try {
            deploy = repositoryService.createDeployment()
                    .addClasspathResource("processes/XCFDJADY.bpmn20.xml")
                    .name("查封登记")
                    .deploy();
        } catch (Exception e) {
            log.error("部署失败", e);
     ...
     */
    @GetMapping("cxthrw")
    public Result<JSONArray> cxthrw(String userId, String processDefinitionKey,
                                    Integer pageNumber, Integer pageSize) {
        if (userId.equals("undefined")) {
            return error(ERROR_PARAM);
        }
    ...
    public Result<JSONArray> zbrwUsers(String taskId) {
        ...
        for (HistoricActivityInstance historicActivityInstance : historyService.createHistoricActivityInstanceQuery()
            ...
            if (activityName != null && !activityName.equals("通过") && !activityName.equals("开始") &&
                ...
                for (String userId : variables.get(convert)
                        .toString()
                        .split(",")) {
