...
/**
 * 国有建设用地使用权及房屋所有权首次登记（商品房），首次登记—多幢，首次登记（厂房）
...
@RestController
@RequestMapping("SCDJ")
public class SCDJFlowableController extends BaseController {
    ...
    public Result<String> qiDong() {
        ...
        try {
            deploy = repositoryService.createDeployment()
                    .addClasspathResource("processes/GYJSYDSYQJFWSYQSCDJDZ.bpmn20.xml")
                    .name("国有建设用地使用权及房屋所有权首次登记（商品房），首次登记—多幢，首次登记（厂房）")
                    .deploy();
        } catch (Exception e) {
            log.error("部署失败", e);
            return error(ERROR_PARAM);
    ...
    @PostMapping("qd2")
    public Result<JSONArray> qd(String key, String name, String slbh, HttpServletRequest request) {
...
//        List<User> list1 = identityService.createUserQuery().memberOfGroup("sjz").list();
//
//        us:
//        {
//            for (User s : list1) {
//                if (user.equals(s.getId())) {
//                    break us;
//                }
//            }
//
...
//        }
     ...
     */
    @PostMapping("qsrw")
    public Result<String> qsrw(String taskId, HttpServletRequest request) {
        String userId1 = null;
        try {
     ...
     */
    @GetMapping("lcmc")
    public Result<JSONArray> getlcmc(String taskId, String slbh, HttpServletRequest request) {
        JSONArray array = new JSONArray();
        List<Lcmc> lcmc = new ArrayList<>();

        if ("undefined".equals(taskId)) {
            lcmc = homePageService.lcmc(slbh);
        } else {
...
//        newList.forEach(l -> array.add(parseObj(l, true, true)));
    ...
    public Result<JSONArray> audit(String variate, String taskId, String taskName,
                                   String users, String name, HttpServletRequest request) {
        String user;
        Task task = taskService.createTaskQuery()
                .taskId(taskId)
                .singleResult();
     ...
     */
    @GetMapping("ls2")
    public Result<JSONArray> ls(String processDefinitionId) {
        List<HistoricActivityInstance> list = historyService.createHistoricActivityInstanceQuery()
                .processDefinitionId(processDefinitionId)
                .finished()
                .orderByHistoricActivityInstanceEndTime()
