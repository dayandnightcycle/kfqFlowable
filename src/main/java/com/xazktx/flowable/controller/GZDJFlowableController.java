...
@RestController
@RequestMapping("GZDJ")
public class GZDJFlowableController extends BaseController {
    ...
    public Result<String> qiDong() {
        ...
        try {
            deploy = repositoryService.createDeployment()
                    .addClasspathResource("processes/GZDJHZM.bpmn20.xml")
                    .name("更正登记（证书含证明）")
                    .deploy();
        } catch (Exception e) {
            log.error("部署失败", e);
     ...
     * @return
     */
    @RequestMapping(value = "sc2", method = RequestMethod.DELETE)
    ...
    @PostMapping("qd2")
    public Result<String> qd(String key, String name, String slbh, String user) {
        ...
        try {
            ...
            holidayRequest = runtimeService.startProcessInstanceByKey(key, variables);
        ...
        log.info("processInstance.getProcessDefinitionId() = {} --- processInstance.getId() = {}",
                holidayRequest.getProcessDefinitionId(), holidayRequest.getId());
